...
export default function DesignFourOnboardingPage() {
  ...
  const handleFinish = () => {
    ...
    router.push("/design-four/dashboard");
  };
  ...
  return (
    <div className="theme-forest flex min-h-screen bg-background">
      {/* Master list — mirrors Design Four's job-list-panel: a slim bordered
          rail of rows, active row highlighted with bg-primary/10. */}
      <aside className="hidden h-screen w-64 shrink-0 flex-col border-r border-border lg:flex">
        <Link
          href="/design-four/dashboard"
          ...
          {STEPS.map((s, index) => {
            ...
            return (
              <button
                ...
                disabled={index > step}
                className={cn(
                  "flex w-full items-start justify-between gap-2 border-b border-border px-4 py-3 text-left transition-colors disabled:cursor-not-allowed",
      ...
      {/* Detail pane — the active step's form. */}
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-12 shrink-0 items-center gap-3 border-b border-border px-4">
          <Link
            href="/design-four/dashboard"
            className="flex items-center gap-2 text-sm font-semibold text-foreground lg:hidden"
          >
            <span className="flex size-6 items-center justify-center rounded-md bg-primary text-xs font-semibold text-primary-foreground">
              A
            </span>
          ...
          <button
            type="button"
            onClick={handleSkip}
            className="ml-auto text-sm text-muted-foreground underline decoration-dotted underline-offset-4 hover:text-foreground"
          >
            Skip onboarding
          </button>
        </header>
